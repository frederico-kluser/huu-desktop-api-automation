...
describe('keyboard-request.dto', () => {
  describe('keyboardTypeSchema', () => {
    test('validates valid type requests', () => {
      ...
      expect(() =>
        keyboardTypeSchema.parse({ text: 'Test', mode: 'perChar', value: 50 }),
      ).not.toThrow();
    });
    ...
    test('rejects invalid text', () => {
      ...
      expect(() => keyboardTypeSchema.parse({ text: longText })).toThrow(
        'Text exceeds maximum length',
      );

      // Apenas caracteres de controle
      expect(() => keyboardTypeSchema.parse({ text: '\x00\x01\x02' })).toThrow(
    ...
    test('validates value constraints', () => {
      // Valor negativo
      expect(() => keyboardTypeSchema.parse({ text: 'Test', value: -1 })).toThrow(
        'Value must be non-negative',
      );

      // Valor não inteiro
      expect(() => keyboardTypeSchema.parse({ text: 'Test', value: 1.5 })).toThrow(
        'Value must be an integer',
      );

      // Valor muito alto
      expect(() => keyboardTypeSchema.parse({ text: 'Test', value: 300001 })).toThrow(
  ...
  describe('keyboardCombinationSchema', () => {
    ...
    test('validates array constraints', () => {
      // Array vazio
      expect(() => keyboardCombinationSchema.parse({ keys: [] })).toThrow(
        'requires at least one key',
      );

      // Muitas teclas
      expect(() =>
        keyboardCombinationSchema.parse({ keys: ['ctrl', 'alt', 'shift', 'cmd', 'a', 'b'] }),
      ).toThrow('maximum 5 keys');
    });

    test('rejects unsupported keys in combination', () => {
      expect(() => keyboardCombinationSchema.parse({ keys: ['ctrl', 'enter'] })).toThrow(
        'Unsupported key in combination',
      );
      expect(() => keyboardCombinationSchema.parse({ keys: ['invalid'] })).toThrow(
        'Unsupported key in combination',
      );
    });
