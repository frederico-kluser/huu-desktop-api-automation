  ...
  container: {
    resolve: jest.fn().mockImplementation((token: string) => {
      if (token === 'KeyboardService') {
        return {
          type: jest.fn().mockResolvedValue({ success: true, data: { textLength: 5, mode: 'instant', timing: 0 } }),
          pressKey: jest.fn().mockResolvedValue({ success: true, data: { key: 'a' } }),
          combination: jest.fn().mockResolvedValue({ success: true, data: { combination: 'ctrl+c' } }),
        };
      }
      if (token === 'ClipboardService') {
        return {
          copy: jest.fn().mockResolvedValue({ success: true, data: { contentLength: 10, sizeBytes: 10 } }),
          paste: jest.fn().mockResolvedValue({ success: true, data: { content: 'test', isEmpty: false, contentLength: 4 } }),
...
describe('KeyboardController', () => {
  ...
  test('registra todas as rotas esperadas', () => {
    ...
    expect(fastifyInstance.post).toHaveBeenCalledWith('/keyboard/type', expect.any(Object), expect.any(Function));
    expect(fastifyInstance.post).toHaveBeenCalledWith('/keyboard/press', expect.any(Object), expect.any(Function));
    expect(fastifyInstance.post).toHaveBeenCalledWith('/keyboard/combination', expect.any(Object), expect.any(Function));
    
    // Verificar rotas de clipboard
    expect(fastifyInstance.post).toHaveBeenCalledWith('/clipboard/copy', expect.any(Object), expect.any(Function));
    expect(fastifyInstance.get).toHaveBeenCalledWith('/clipboard/paste', expect.any(Object), expect.any(Function));
    expect(fastifyInstance.post).toHaveBeenCalledWith('/clipboard/clear', expect.any(Object), expect.any(Function));
  });
      ...
      container.resolve = jest.fn().mockImplementation((token: string) => {
        if (token === 'KeyboardService') {
          return {
            ...
            combination: jest.fn().mockResolvedValue({ success: false, error: 'Combination error' }),
          };
        }
        if (token === 'ClipboardService') {
          return {
