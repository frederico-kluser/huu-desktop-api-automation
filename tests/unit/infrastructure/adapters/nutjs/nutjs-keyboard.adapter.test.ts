...
const { NutJSKeyboardAdapter } = require('../../../../../src/infrastructure/adapters/nutjs/nutjs-keyboard.adapter');
...
describe('NutJSKeyboardAdapter', () => {
  ...
  describe('pressKey', () => {
    ...
    it('should throw error for unsupported key', async () => {
      await expect(adapter.pressKey('unsupported')).rejects.toThrow("Failed to press key 'unsupported': Unsupported key: unsupported");
    });

    it('should throw error when pressKey fails', async () => {
      mockKeyboard.pressKey.mockRejectedValueOnce(new Error('Press error'));
      
      await expect(adapter.pressKey('enter')).rejects.toThrow("Failed to press key 'enter': Press error");
    });

    it('should handle non-Error thrown values', async () => {
      mockKeyboard.pressKey.mockRejectedValueOnce('string error');
      
      await expect(adapter.pressKey('enter')).rejects.toThrow("Failed to press key 'enter': Unknown error");
    });
  });
  ...
  describe('releaseKey', () => {
    ...
    it('should throw error for unsupported key', async () => {
      await expect(adapter.releaseKey('invalid')).rejects.toThrow("Failed to release key 'invalid': Unsupported key: invalid");
    });

    it('should throw error when releaseKey fails', async () => {
      mockKeyboard.releaseKey.mockRejectedValueOnce(new Error('Release error'));
      
      await expect(adapter.releaseKey('tab')).rejects.toThrow("Failed to release key 'tab': Release error");
    });

    it('should handle non-Error thrown values', async () => {
      mockKeyboard.releaseKey.mockRejectedValueOnce(123);
      
      await expect(adapter.releaseKey('tab')).rejects.toThrow("Failed to release key 'tab': Unknown error");
    });
  });
  ...
  describe('combination', () => {
    ...
    it('should throw error for unsupported key in combination', async () => {
      await expect(adapter.combination(['ctrl', 'invalid'])).rejects.toThrow('Failed to perform key combination: Unsupported key in combination: invalid');
    });
    ...
    it('should throw error when pressKey fails in combination', async () => {
      ...
      await expect(adapter.combination(['ctrl', 'c'])).rejects.toThrow('Failed to perform key combination: Press failed');
    });
    ...
    it('should throw error when releaseKey fails in combination', async () => {
      ...
      await expect(adapter.combination(['ctrl', 'c'])).rejects.toThrow('Failed to perform key combination: Release failed');
    });
    ...
    it('should handle non-Error thrown values in combination', async () => {
      ...
      await expect(adapter.combination(['ctrl', 'c'])).rejects.toThrow('Failed to perform key combination: Unknown error');
    });
