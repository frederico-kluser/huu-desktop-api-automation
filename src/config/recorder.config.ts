...
function validateConfig(): RecorderConfig {
  ...
  if (moveIntervalMs < 20 || moveIntervalMs > 200) {
    throw new Error(
      `RECORDER_MOVE_INTERVAL_MS deve estar entre 20 e 200ms (valor: ${moveIntervalMs})`,
    );
  }

  const maxScreenshotSize = parseInt(process.env.RECORDER_MAX_SCREENSHOT_SIZE || '2097152', 10); // 2MB default
  if (maxScreenshotSize < 10240 || maxScreenshotSize > 10485760) {
    // 10KB - 10MB
    throw new Error(
      `RECORDER_MAX_SCREENSHOT_SIZE deve estar entre 10KB e 10MB (valor: ${maxScreenshotSize})`,
    );
  }
